...
async function asyncSaveEvents(events: RecordedHostEvent<any>[], hostBatchWriteId: number): Promise<void> {
  await database.hostEvent.create({
    ...
    savePromise
      ...
      .catch((e) => {
        if (onRetry) {
          ...
          console.error(e);
          console.log(`=== ERROR Saving Host Events (${hostBatchWriteId}) ===: ` + JSON.stringify(eventsToSave));
        }
      })
      .finally(() => {
...
export function writeEvent<EvtTypeName extends HostEventType>(type: EvtTypeName, event: HostEvents[EvtTypeName]): void {
  events.push({
    type,
    event,
    time: Date.now(),
